...
namespace TestFramework {
	...
	CommandPool::CommandPool(Device* InDevice, const CommandPoolInfo& InCreateInfo)
	    : m_CreateInfo(InCreateInfo), m_Device(InDevice)
	{
		...
		vkCreateCommandPool(InDevice->GetLogicalDevice(), &PoolInfo, nullptr, &m_CommandPool);
		...
		for (uint32_t Idx = 0; Idx < InCreateInfo.InitialListCount; Idx++)
		{
			...
			vkAllocateCommandBuffers(m_Device->GetLogicalDevice(), &AllocInfo, &CmdBuffer);
