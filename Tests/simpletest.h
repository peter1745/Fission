...
// NOTE(Peter): Simple Test Framework: https://github.com/kudaba/simpletest
...
#define DEFINE_TEST_FULL(name, group, fixture)     \
	...
	{                                              \
		char const* TestName() const override      \
		{                                          \
			return #name;                          \
		}                                          \
		char const* TestGroup() const override     \
		{                                          \
			return #group;                         \
		}                                          \
...
#define TEST_OPERATOR(a, b, op1, op2)                                                                                                                            \
	TEST_BEGIN_(a);                                                                                                                                              \
	TEST_CHECK_((test_value_) op1(b), STR(a) " " STR(op1) " " STR(b), "'%s' " STR(op2) " '%s'", TEST_TYPE_TO_STRING(test_value_, a), TEST_TYPE_TO_STRING(b, b)); \
	TEST_END_
...
#define TEST_STR_EQ(a, b)                                                                                                                 \
	do {                                                                                                                                  \
		if (!TestFixture::GetCurrentTest()->TestStrings(a, b, TEST_ERROR_PREFIX_ "\n%s\n%s\n%s", STR(a) " == " STR(b))) { ERROR_ACTION; } \
	} while (0)
#define TEST_CLOSE(a, b, eps)                                                                                                                                                                                                                                               \
	TEST_BEGIN_(TestDifference(a, b));                                                                                                                                                                                                                                      \
	TEST_CHECK_(test_value_ <= eps, STR(a) " Close to " STR(b), "Difference of %s is greater than expected amount of " STR(eps) " when comparing %s and %s", TEST_TYPE_TO_STRING(test_value_, TestDifference(a, b)), TEST_TYPE_TO_STRING(a, a), TEST_TYPE_TO_STRING(b, b)); \
	TEST_END_
#define TEST_DIFFERS(a, b, eps)                                                                                                                                                                                                                                              \
	TEST_BEGIN_(TestDifference(a, b));                                                                                                                                                                                                                                       \
	TEST_CHECK_(test_value_ >= eps, STR(a) " Differs from " STR(b), "Difference of %s is less than expected amount of " STR(eps) " when comparing %s and %s", TEST_TYPE_TO_STRING(test_value_, TestDifference(a, b)), TEST_TYPE_TO_STRING(a, a), TEST_TYPE_TO_STRING(b, b)); \
	TEST_END_
